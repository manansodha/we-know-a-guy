...
class BelimoAgent:
    ...
    def analyze_physics(self, current_torque):
        ...
        if len(self.torque_memory) >= 10:
            ...
            # A) anomaly detected
            # TODO: Add also analysis with delta of expected and actual position
            # 1) The set position is never reached
                # => torque is just too high for too long, but not spiking. 
                # This could be a sign of a mechanical blockage or wear.
            # 2) The set position is overshooting and oscillating around the target
                # => This could be a sign of a control system instability, maybe due to a failing
            # 3) The torque is normal, but the internal temperature is rising steadily
            # 4) For temperature sensors / water pipe control:
            #    The temperature is not cooling down, even if the valve is fully open.

            # B) Trying a local fix.

            # C) Alert the swarm if the anomaly persists for a certain deadline.
